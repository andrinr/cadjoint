...
[![The cadjoint playground: the scene program on the left, the parametric heat sink rendered live on the right, and the declared optimization in the side dock](docs/assets/screens/model-desk.png)](docs/viewer.qmd)
...
| **[Plugins](docs/plugins.qmd)** | Meshers and solvers behind one `apply`/`vjp` contract, so one `jax.grad` crosses every tool boundary |
