...
cadjoint combines parametric geometry, geometric constraints, and automatic differentiation to enable gradient-based shape optimization.

[![CADJOINT WebGPU playground with Python source beside a live rendered scene](examples/assets/webgpu_playground.png)](docs/viewer.qmd)

Build a scene in Python and inspect the generated WGSL in a live, interactive
WebGPU preview. [Open the WebGPU viewer guide](docs/viewer.qmd).

For still images, the [forward renderer guide](docs/rendering.qmd) covers the
draft, balanced, and high-quality presets with real output comparisons.
...
| **Rendering** | Forward-only sphere tracing with explicit performance and fidelity settings |
