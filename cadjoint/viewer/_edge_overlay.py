...
def _prune_debris(entries: list[_Drawable], limit: float) -> list[_Drawable]:
    ...
    for (edge, samples), pair in zip(entries, ends):
        root = find(pair[0])
...
def _sharp_polylines(brep: BRep, spacing: np.ndarray) -> list[tuple[BRepEdge, np.ndarray]]:
    ...
    * and what comes out does not fold — :data:`_MAX_EDGE_TURN`, which is
    ...
    Returns:
        One ``(edge, points)`` pair per drawn edge; a closed edge's points
    ...
    entries = [
        _Drawable(edge, polylines[edge.index])
...
def _sharp_chords(brep: BRep, spacing: np.ndarray) -> np.ndarray:
    """The drawn edges as chord pairs, in the payload's shape.
    ...
    """
    chords = []
    for edge, points in _sharp_polylines(brep, spacing):
