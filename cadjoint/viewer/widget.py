...
try:
    ...
    raise ImportError(
        ...
        "Install them with: pip install 'cadjoint[viewer]'"
    ) from exc
...
_ESM = r"""
// ── shader builder ────────────────────────────────────────────────────────────
const shaderTemplate = __CADJOINT_SHADER_TEMPLATE__;

function buildShader(sdfCode) {
  return shaderTemplate.replace('__CADJOINT_SDF_CODE__', sdfCode);
}
...
_ESM = _ESM.replace("__CADJOINT_SHADER_TEMPLATE__", json.dumps(WGSL_VIEWER_TEMPLATE), 1)

# ── Python widget ─────────────────────────────────────────────────────────────


def _compile_viewer_wgsl(fn: Callable) -> str:
    """Compile complete CADJOINT scenes while retaining plain-callable support."""
