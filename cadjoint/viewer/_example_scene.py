"""The program the playground opens with.

``EXAMPLE_SOURCE`` is the starter scene ``GET /api/session`` hands the
frontend: a parametric heat sink that exercises the whole toolchain at once
— a constrained sketch, extrusion and revolve, a named ``SimMesh``, a
thermal study, and one declared optimization that descends it.  It is
program text the worker executes later, never code this module runs, so it
lives on its own instead of crowding the server.
"""
