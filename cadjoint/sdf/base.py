...
def _child_patch_fields(child) -> list | None:
    ...
    A node's child may be a plain callable rather than an SDF node — both
    :meth:`cadjoint.sdf.transforms._operation._Operation.children` and the
    boolean operands tolerate that — and a bare callable declares no patches,
    so there is nothing for the parent to forward either.  Written as an
    attribute lookup rather than a call on :class:`SDF` so every node that
    forwards the protocol shares one guard.

    Lives here, beside the protocol itself, because both the transforms and
    the booleans need it and neither subpackage should import the other.
