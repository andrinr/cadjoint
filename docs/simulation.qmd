...
There is no fourth `method`. Tets sized by the part rather than by the lattice,
with quadratic midside nodes on the true surface, come from Gmsh on the
[derived B-rep](brep.qmd#meshing-from-the-graph); that route is reached through
`cadjoint.brep.mesh_gmsh` and the `tet_mesher` plugin, and is not yet a
`SimMesh` keyword.
