...
edges: each is the exact intersection curve of the two surfaces it separates,
sampled at half a cell and projected onto both, so a bore rim is a circle whose
radius is right to a millionth of a unit rather than a staircase, and it stays
that circle wherever the grid cells happen to fall. Curves run into their corners and meet there
exactly. Where a fillet rounds a corner away the graph says so and nothing is
drawn: the model has no edge there. See
...
`tests/viewer/test_parity_roundtrip.py` closes the loop for all 27 operations:
...
they mount again — so leaving Simulate mode and coming back no longer costs
a second solve. A result whose hash no longer matches the editor is kept and
