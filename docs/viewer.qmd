...
line is drawn inside it. That behaviour belongs to the `feature_edges`
[plugin kind](plugins.qmd#the-five-in-process-kinds); without a provider the overlay
falls back to the lattice classifier and the compile payload says `lattice`.
