"""Horizontal constraint between two sketch points."""
...
class HorizontalConstraint(Constraint):
    """Constraint that forces two sketch points onto a horizontal line.

    This constraint enforces: y1 = y2 (in sketch-plane coordinates).
    ...
    Example:
        ...
        p2 = Vector2([2, 1], free=True, name='p2')
        constraint = HorizontalConstraint(p1, p2)
        ```
    """
    ...
    def compute_residual(self, param_values: dict[str, Array]) -> Array:
        """Compute horizontal constraint residual: y1 - y2.
        ...
        return p1_val[1] - p2_val[1]

    def dof_reduction(self) -> int:
        """Horizontal constraint adds 1 scalar equation."""
        return 1

    def get_parameters(self) -> list[Parameter]:
        """Return both points involved in the horizontal constraint."""
        return [self.point1, self.point2]

    def __repr__(self) -> str:
        return f"HorizontalConstraint({self.point1.name}, {self.point2.name})"
