...
# ── Simulation studies ───────────────────────────────────────────────────────
# Studies are first-class code citizens (jaxcad.fem.study): the viewer edits
...
def _selection_source(description) -> str:
    ...
    Round-trips through :func:`jaxcad.fem.selection.selection_from_description`
    ...
    """
    from jaxcad.fem.selection import selection_from_description
...
def add_study(source: str, kind: str, name: str | None = None) -> str:
    ...
    imports the constructor from ``jaxcad.fem`` beside it, keeping every line
    ...
    patched = source[:insert] + statement + source[insert:]
    patched = _ensure_import(
        patched, ast.parse(patched), "jaxcad.fem", symbol, prefer_offset=insert
    )
    return _validate(patched)
...
def add_study_bc(source: str, study, bc_type: str, selection, value=None) -> str:
    ...
    class and ``Nodes`` are imported from ``jaxcad.fem`` beside the study so
    ...
    patched = _ensure_import(
        patched, ast.parse(patched), "jaxcad.fem", symbol, prefer_offset=import_offset
    )
    patched = _ensure_import(
        patched, ast.parse(patched), "jaxcad.fem", "Nodes", prefer_offset=import_offset
    )
    return _validate(patched)
