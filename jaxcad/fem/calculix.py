...
HEX8 meshes from :func:`jaxcad.fem.sdf_to_hex_mesh` serialize 1:1 to C3D8
...
The ccx binary is located via the ``JAXCAD_CCX`` or ``CCX`` environment
...
from jaxcad.fem.backends import _TESSERACT_EXTRA_MESSAGE, ElasticBCs, TesseractBackend, _x64_scope
from jaxcad.fem.hexmesh import _boundary_face_rows
...
_CCX_INSTALL_MESSAGE = (
    "CalculiX binary (ccx) not found. Point the JAXCAD_CCX or CCX environment "
...
def find_ccx(explicit: str | os.PathLike | None = None) -> str | None:
    ...
    Resolution order: ``explicit`` argument, ``JAXCAD_CCX`` env var,
    ...
    """
    for candidate in (explicit, os.environ.get("JAXCAD_CCX"), os.environ.get("CCX")):
...
def elastic_ccx_solve(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="jaxcad-ccx-") as temporary:
...
class CalculixBackend(TesseractBackend):
    ...
    ``jaxcad/fem/tesseracts/elastic_calculix`` (subprocess ccx, local
    ...
    def __init__(self, ccx: str | os.PathLike | None = None):
        ...
        if ccx is not None:
            os.environ["JAXCAD_CCX"] = str(ccx)  # the tesseract subprocess resolves via env
...
def strain_energy_solve(
    ...
    *,
    ...
    The patch-resolution twin of :func:`jaxcad.fem.simulate.elastic_solve`
